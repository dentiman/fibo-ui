...
`MultiSelect` is a compact blueprint built from the same CDK primitives as `Select` — `DataList` with `SelectMulti` — plus chips and checkboxes. Copy it as-is, or recompose those primitives into your own multi-value selector.

:::example recipe

```ts {example="recipe" title="multi-select.ts"}
...
```html {example="recipe" title="multi-select.html"}
<fibo-field-shell
  [label]="label()"
  [hint]="hint()"
