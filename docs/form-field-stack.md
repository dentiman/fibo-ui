...
| `FieldContext` | `[fiboFieldContext]` | — | `data-size` (via `Size` hostDirective), `data-label-layout` | — |
...
`FieldContext` ставиться на зовнішньому обгортці споживача. Його `data-size` (через `Size` hostDirective — спільна примітив з `Button`) / `data-label-layout` каскадуються CSS descendant selectors всередину `.fibo-field-container`:
```css
[data-size="sm"] .fibo-field-container { --ff-control-min-height: 2rem; }
[data-size="lg"] .fibo-field-container { --ff-control-min-height: 2.75rem; }
...
Ставиться на зовнішньому контейнері. Контролює size (через `Size` hostDirective — **той самий примітив, що й у `Button`**, гарантує однакові назви розмірів `sm` / `md` / `lg`) і label-layout.
