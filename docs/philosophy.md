...
#### Приклад: Select і Menu з одного DataList

Найяскравіше це видно на парі **Select** і **Menu**. Обидва — списки, якими керуєш з клавіатури: стрілки вгору/вниз, Home/End, Enter, tracking активного пункту. Уся ця логіка написана **один раз** — у примітиві `DataList` — і жоден з компонентів її не дублює:

```
Select = DataList + SelectOne  + FieldShell        → повертає значення у форму
Menu   = DataList + MenuPanel  + дія / навігація    → виконує команду
```

- `DataList` дає обом **однакову** клавіатурну навігацію; `DataListItem` на кожному пункті — теж спільний і працює однаково для опції Select і для пункту Menu.
- Далі композиція розходиться: Select додає `SelectOne` (вибір значення) та `FieldShell` (обгортку поля форми з label і валідацією); Menu загортає той самий `DataList` у `MenuPanel` (timing для submenu та hover), а пункти виконують дію чи навігацію замість того, щоб зберігати значення.

Результат — два зовсім різні компоненти (один віддає значення у форму, інший виконує команду), але навігація в них **буквально той самий код**. Виправлення чи покращення в `DataList` миттєво отримують обидва — і Table, і DatePicker разом з ними. А якщо потрібен свій `CommandPalette` чи `UserPicker` — береш той самий `DataList`, додаєш власну обгортку, і клавіатурна навігація вже працює.
